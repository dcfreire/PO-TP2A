...
class Model:
    ...
    def add_constraint(self, constraint: np.ndarray, b: np.longdouble, is_eq=False):
        self.constraints.append(constraint)
        self.b.append((b, is_eq))
    ...
    def _create_tableau(self):
        ...
        self.tableau[0, :] = line
        for i, (constraint, (b, is_eq)) in enumerate(zip(self.constraints, self.b)):
            unit = [0] * len(self.constraints)
            unit[i] = 1
            if is_eq:
                line = np.hstack([unit, constraint, [0] * len(self.constraints), b])
            else:
